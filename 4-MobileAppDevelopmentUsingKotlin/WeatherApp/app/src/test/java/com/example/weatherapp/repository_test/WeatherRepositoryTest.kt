...
class WeatherRepositoryTest {
    ...
    private val fakeHourlyForecastResponse = WeatherResponse(
        id = 0,
        cod = "200",
        message = 0,
        numberOfHours = 1,
    ...
    private val fakeFavouriteCountry = FavouriteCountry(
        name = "Tokyo",
        latitude = 35.6895f,
        longitude = 139.6917f,
        minTemp = 285.15f,
    ...
    private val fakeFavouriteCountry2 = FavouriteCountry(
        name = "EGYPT",
        latitude = 35.6895f,
        longitude = 139.6917f,
        minTemp = 285.15f,
    ...
    @Before
    fun setup() {
        val weatherResponses = mutableMapOf(Pair(51.5085f, -0.1257f) to fakeHourlyForecastResponse)
        val currentWeatherResponses = mutableMapOf(Pair(51.5085f, -0.1257f) to fakeCurrentWeatherResponse)
    ...
    fun getHourlyForecast_validCoordinates_returnsSuccess() = runTest {
        ...
        assertThat(result.isSuccess, `is`(true))
        result.onSuccess { response ->
            assertThat(response.city.name, `is`(not(1.23f)))
    ...
    fun saveHourlyForecastToLocal_savesCorrectly() = runTest {
        repo.saveWeatherResponse(fakeHourlyForecastResponse)
    ...
    fun saveCurrentWeatherToLocal_savesCorrectly() = runTest {
        repo.saveCurrentWeatherResponse(fakeCurrentWeatherResponse)
    ...
    fun favoriteLocationOperations_workCorrectly() = runTest {
        // Test insert
        repo.insertFavouriteCountry(fakeFavouriteCountry)
        val locations = repo.getAllFavouriteCountries()
        assertThat(locations.size, `is`(1))
        assertThat(locations[0].name, `is`("Tokyo"))

        // Test delete
        repo.deleteFavouriteCountry(fakeFavouriteCountry.name, fakeFavouriteCountry.latitude, fakeFavouriteCountry.longitude)
    ...
    fun alertOperations_workCorrectly() = runTest {
        // Test save
        val alertId = repo.insertAlert(fakeAlert)
    ...
    @Test
    fun alertOperations_doesNotWorkCorrectly() = runTest {

        val alertId = repo.insertAlert(fakeAlert)

        val retrievedAlert = repo.getAlertById(alertId)

        val incorrectDateMillis = 1234567890000L

        assertThat(retrievedAlert?.dateMillis, `is`(not(incorrectDateMillis)))
    }

    @Test
    fun favoriteLocationOperations_doesNotWorkCorrectly() = runTest {
        // Test insert
        repo.insertFavouriteCountry(fakeFavouriteCountry) // Country "Tokyo" with coordinates (35.6895, 139.6917)
        repo.insertFavouriteCountry(fakeFavouriteCountry2)
        val locations = repo.getAllFavouriteCountries()
        val incorrectCountryName = "New York"
        assertThat(locations[0].name, `is`(not(incorrectCountryName)))
        assertThat(locations.size, `is`(not(1)))
    }
}